...
| 時間割 DB（`schedule_final.db`） | リポジトリ内 | **手作業**。2026-06-10 にアップロードされたもの。前期 6,897 行・後期 6,717 行・教室 181。**リポジトリ内のファイルからは再現できない**（6. 参照） |
...
- **時間割 DB の作り方が失われている（最重要）。** `schedule_final.db` は 2026-06-10 にアップロードされたもので、作成手順もスクリプトも残っていない。`data/source/*.xlsx` は原本ではなく集計結果で、2026-09-11 に突き合わせたところ **DB の占有スロット 3,616 件のうち xlsx から導けるのは 1,732 件（48%）** だった。DB には (a) 学科・科目名（xlsx に無い。`元ファイル` 列の `1-07.xlsx` のような**学科ごとの時間割ファイル**が本当の原本）、(b) 教室名の手作業正規化（`1041･1042`→`1042/43` など）、(c) タワースコラの分類（xlsx は船橋／駿河台の2区分のみ）が含まれる。
  → 翌年度（2027-04）に向けて **原本ファイル一式と正規化ルールを回収し、`scripts/build_schedule_db.py` として再現可能にする**こと。原本の所在: `[要記入]`。DB を差し替えるときは `tests/test_schedule_db.py` が最低限の整合性（両学期・3校舎・教室マスタとの対応）を検査する。
...
- [ ] 時間割 DB の原本ファイル（学科ごとの時間割 xlsx）と作成手順の所在を確認し、6. の `[要記入]` を埋めた
- [ ] 6. の未整備事項を口頭でも共有した
