...
- `schedule_final.db` — 時間割DB。**複数年度を同居させ、`年度` 列を持つ**（`schedules.年度` / `classrooms.年度`）。教室は年度で増減するので、教室マスタも年度ごと。アプリは年度と学期を利用者に選ばせ、既定は「いまの年度・いまの学期」。原本は `data/source/<年度>/` の `<区分>_<学部>_<学科>_..._時間割表N面_*.xls`（教務ページ配布の時間割表。区分 1=理工学部 2=短期大学部 3=博士前期 4=博士後期）。原本が入っている年度フォルダを**全部**読む（`--year` で1年度に絞れる）。`python scripts/build_schedule_db.py --report` で原本から作り直せる。既定では本番DBを上書きせず `schedule_final.new.db` に出す。教室名は**全角・半角の違いを NFKC で統一**し、「1456/7/8」のような**合同教室の略記は 1456・1457・1458 に分けて**入れる（2026-09、ユーザー指示。`build_schedule_db.py` の規則D・`ROOM_FIXES`）。本番DBは全区分のビルド結果と完全に一致する（`test_full_build_reproduces_production_db_exactly`）。差し替え時は `tests/test_schedule_db.py` と `tests/test_build_schedule_db.py` が検査する。手順は `docs/OPERATIONS.md`。
...
- `reservations.db` / `reports.db` — 実行時に自動生成される揮発データ。**コミットしない**（.gitignore済み）再デプロイで消えるので、**アプリに影響する push を授業時間中（平日 9:00–20:30）にするときは、仮予約・報告が消えることをユーザーに伝えてから行う**（永続ディスクは付けない方針）。
- `data/*.json` — Instagram/GA4 の集計。**2026-08 で収集を終了**し、ダッシュボード表示用に凍結（更新しない。収集用のワークフロー・スクリプトは撤去済み）。

## 守るべきルール

1. **「テスト運用中・非公式」表記を消さない**: 検索アプリの画面（`templates/index.html`・`room.html`・`room_search.html`）・LP（`index.html`）・dashboard.html・ポスターに常設。学生課/教務課と協議中のため誤認防止が必須。
...
5. **本番（`main`）に出す前に、運営者の確認と承認を取る**（2026-09 ユーザー指示）。利用者に見える変更（アプリ・LP・ダッシュボード・時間割DB）は、まず作業ブランチにコミット・push し、変わる画面のスクショ（390px と PC 幅。`/verify-ui`）を SendUserFile で送り、何が変わるか・判断が要る点を伝える。**「OK」と明示されてから** `main` に push する。直しを頼まれたら直して再提示する。授業時間中（平日 9:00–20:30）に承認されたアプリ変更は、仮予約・報告が消えることを伝えて、今出すか授業後にするかを確認する。docs・テストだけの変更はこの確認なしでよいが、Render の再デプロイを避けるため授業時間外に出す。
